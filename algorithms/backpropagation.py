...
def Backpropagation(hidden_layer_number, neurons_list, eta_entry, epochs_entry, bias_var, activation_var):
    ...
    # parameters of confusion matrix in training data
    ...
    # parameters of confusion matrix in testing data
    bombay_predbombay = 0
    bombay_predcali = 0
    bombay_predsira = 0
    cali_predbombay = 0
    ...
    testing_accuracy = (bombay_predbombay + cali_predcali + sira_predsira) / len(y_test)
    print(f"testing accuracy: {round(testing_accuracy * 100,2)}% ")
    confusion_matrix = np.array([[bombay_predbombay, cali_predbombay, sira_predbombay], [bombay_predcali, cali_predcali, sira_predcali],
                                 [bombay_predsira, cali_predsira, sira_predsira]])
                ...
                for neuron in range(len(generated_weights[i])):
                    for weight in range(len(generated_weights[i + 1])):
                        if bias == 0:
                            ...
                            sigma_neuron += result[0][weight] * generated_weights[i + 1][weight][neuron + 1]
                    # the result of dot product multiply derivative of activation function
                    sigma = sigma_neuron * (1 - (Net_answers[i][neuron]**2))
                    sigma_layer.append(sigma)
                result.insert(0, sigma_layer)
