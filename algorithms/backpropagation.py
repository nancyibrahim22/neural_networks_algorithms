...
def Backpropagation(hidden_layer_number, neurons_list, eta_entry, epochs_entry, mse_entry, bias_var, activation_var):
    ...
    for epoch in range(epochs_entry):
        for pp in range(90):
            ...
            for i in range(hidden_layer_number+1):
                if i ==0:
                    Net_answers.append(CalcNet(i,neurons_list[i],x_train_scaled[pp],activation_var))
                elif i==hidden_layer_number:
    ...
    for o in range(60):
        ...
        for i in range(hidden_layer_number + 1):
            if i == 0:
                Net_answers2.append(CalcNet(i, neurons_list[i], x_test_scaled[o], activation_var))
            elif i == hidden_layer_number:
                X = CalcNet(i, 3, Net_answers2[i - 1], activation_var)
                maximum=max(X)
